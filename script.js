...
function serializeState() {
  return {
    ...
    territories: G.territories.map(t => ({
      id: t.id, owned: t.owned, relation: t.relation, atWar: t.atWar,
...
function restoreState(d) {
  G = {
    ...
    territories: TERRITORIES.map(base => {
      ...
      return {
        ...base, owned: !!s.owned, relation: s.relation || 'neutral', atWar: !!s.atWar,
...
function startGame() {
  ...
  initGame(s); showScreen('game'); render(); startBgMusic(); scheduleRandomMeows();
...
function initGame(s) {
  ...
  G = {
    ...
    units: UNITS.map(u => ({ ...u, count: u.id === 'house' ? 2 : 0 })),
...
function endTurn() {
  ...
  G.territories.filter(t => !t.owned && !t.atWar && t.army > 8).forEach(t => {
    ...
    if (Math.random() < aggression) {
      ...
      news.push({
        t: '<i class="bi bi-lightning-charge-fill"></i> ' + t.displayName + ' Declares War!',
...
// ══════════════════════════════════════════════
function render() {
  updateMapColors(); renderCatPanel(); renderTopbar(); renderTokens(); renderArmy(); 
...
function renderArmy() {
  const html = G.units.map(u => `
    <div class="card">
      ...
      <div class="cs">Count: <b>${u.count}</b></div>
      <div class="cg">Cost: ${u.cost} fish each</div>
      <div class="cg">${u.desc || ''}</div>
    </div>
  `).join('');
  document.getElementById('army-list').innerHTML = html;
}
...
function getCatImagePath() {
  const gov = G.gov.toLowerCase();
  const state = G.catState.isNapping ? `-sleeping` : '';
  return `./assets/cat${state}-${gov}.png`;
}

function toggleCatNap() {
  G.catState.isNapping = !G.catState.isNapping;
  G.catState.nappingSince = G.turn;
  if (G.catState.isNapping) {
    addLog('The cat takes a well-deserved nap. 😴', 'ev');
  } else {
    addLog('The cat stretches and yawns. Time to conquer! 🐱', 'ev');
  }
  playCatMeow();
  render();
}
...
function renderCatPanel() {
  ...
  console.log('renderCatPanel called, panel:', panel); // DEBUG
  if (!panel) {
    console.warn('cat-panel not found in DOM'); // DEBUG
    return;
  }
  console.log('Cat image path:', getCatImagePath()); // DEBUG
  
  const img = document.createElement('img');
  img.src = getCatImagePath();
  img.alt = 'Your empire cat';
  img.style.cssText = `
    width: 100%;
    max-width: 120px;
    cursor: pointer;
  ...
  status.style.cssText = `
    ...
    white-space: nowrap;
  `;
  status.textContent = G.catState.isNapping 
    ? `😴 Turn ${G.catState.nappingSince}`
    : '🐱 Click to pet';
...
function startBgMusic() {
  ...
  const purr = document.getElementById('cat-purr');
  if (music) {
    music.currentTime = 0;
    music.play().catch(() => {});
  }
...
function scheduleRandomMeows() {
  audioSettings.meowScheduled = true;
  const delay = 25000 + Math.random() * 15000;
  setTimeout(() => {
    if (audioSettings.meowsEnabled && !document.getElementById('bg-music').paused) {
      playRandomMeow();
      scheduleRandomMeows();
    } else {
      audioSettings.meowScheduled = false;
    }
  }, delay);
}
...
function openSettings() {
  ...
  document.getElementById('music-toggle').checked = audioSettings.musicEnabled;
  document.getElementById('purr-toggle').checked = audioSettings.purrEnabled;
  document.getElementById('meow-toggle').checked = audioSettings.meowsEnabled;
  document.getElementById('music-volume').value = document.getElementById('bg-music').volume * 100;
  document.getElementById('purr-volume').value = document.getElementById('cat-purr').volume * 100;
}

function toggleMusic() {
  audioSettings.musicEnabled = document.getElementById('music-toggle').checked;
  const music = document.getElementById('bg-music');
...
function togglePurr() {
  audioSettings.purrEnabled = document.getElementById('purr-toggle').checked;
  const purr = document.getElementById('cat-purr');
  if (audioSettings.purrEnabled) {
    purr.play().catch(() => {});
  } else {
    purr.pause();
  }
...
function toggleMeows() {
  audioSettings.meowsEnabled = document.getElementById('meow-toggle').checked;
  if (audioSettings.meowsEnabled && !audioSettings.meowScheduled) {
    scheduleRandomMeows();
  }
}

function updateMusicVolume() {
  const val = document.getElementById('music-volume').value / 100;
  document.getElementById('bg-music').volume = val;
}

function updatePurrVolume() {
  const val = document.getElementById('purr-volume').value / 100;
  document.getElementById('cat-purr').volume = val;
}

function restartGame() { stopBgMusic(); showScreen('home'); }